...
<!-- gentle-ai:claude-code-pro-capabilities -->
## Claude Code Pro Capabilities (auto-available)

The following capabilities are active in all sessions:

- **SDD Agents (10)**: sdd-init, sdd-explore, sdd-propose, sdd-spec, sdd-design, sdd-tasks, sdd-apply, sdd-verify, sdd-archive, sdd-onboard — full Spec-Driven Development pipeline
- **MCP Servers (5)**: Engram (persistent memory), Context7 (docs), Exa (websearch), Supabase (database), OpenRouter (multi-model)
- **Skills (24)**: All SDD phases + branch-pr, chained-pr, go-testing, judgment-day, etc.
- **Slash Commands (9)**: /sdd-new, /sdd-ff, /sdd-continue, /sdd-apply, /sdd-verify, /sdd-archive, /sdd-explore, /sdd-init, /sdd-onboard
- **Hooks**: SessionStart (caveman mode), PostToolUse (auto-format: prettier, gofmt, ruff, ktlint, rustfmt)
- **Thinking Mode**: Native extended thinking (Claude Pro subscription)
- **Memory**: Engram persistent memory + Claude's native memory
- **Plugins**: caveman, engram

### Model Assignments (automatic — set in each agent's frontmatter)

| Agent | Model | Reason |
|-------|-------|--------|
| sdd-propose | sonnet | Architectural decisions |
| sdd-design | sonnet | Technical design |
| sdd-spec | sonnet | Structured writing |
| sdd-explore | sonnet | Codebase investigation |
| sdd-tasks | haiku | Mechanical breakdown |
| sdd-apply | sonnet | Implementation |
| sdd-verify | sonnet | Validation |
| sdd-archive | haiku | Close/archive |
| sdd-init | haiku | Bootstrap |
| sdd-onboard | sonnet | Walkthrough |
<!-- /gentle-ai:claude-code-pro-capabilities -->
